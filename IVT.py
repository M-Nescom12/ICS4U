...
class IVT:
    ...
    def find1Zero(self, _x1: float, _x2: float):
        ...
        if deg == -1:
            ...
            z1 = self.brute(x1, x2)
            if z1 is None or abs(self.__Poly.f(z1)) > 1e-5 or (x1 - 1) > z1 > (x2 + 1):  
                z2 = self.newton(x1)
                if z2 is None or abs(self.__Poly.f(z2)) > 1e-5 or (x1) > z2 > (x2 + 1):
                    z3 = self.newton(x2)
                    if z3 is None or abs(self.__Poly.f(z3)) > 1e-5 or (x1 - 1) > z3 > (x2 + 1):
